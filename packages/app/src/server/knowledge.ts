...
import { readKnowledgePageContent, deriveKnowledgeGraph, listWikiEntries } from '@withy/core';
 ...
 * 以便红边两端都有节点可渲染。纯函数,供单测。
 *
 * @param graph core `deriveKnowledgeGraph` 结果
...
export function adaptKnowledgeGraph(graph: KnowledgeGraph, scopeKind: 'global' | 'project'): KnowledgeGraphView {
  const nodes: KnowledgeGraphView['nodes'] = graph.nodes.map(node => ({
    id: node.id,
    label: node.title,
    kind: node.kind,
    scope: node.scope,
  ...
  const edges: KnowledgeGraphView['edges'] = graph.edges.map((edge, index) => ({
    id: `e${index}`,
    source: edge.from,
    target: edge.to,
  ...
  for (const edge of graph.edges) {
    if (edge.broken && !known.has(edge.to)) {
      known.add(edge.to);
...
/** 项目 scope 的关系图(xyflow 友好;broken 边标红)。 */
export function getKnowledgeGraph(scope: Scope): KnowledgeGraphView {
  return adaptKnowledgeGraph(deriveKnowledgeGraph(scope), scope.kind);
}
