...
import type { InitConfig } from '@withy/core';
...
// 知识库根索引种子:占位,由 \`withy knowledge index\` 据 frontmatter 重算(knowledge.md §6.1)
const KNOWLEDGE_INDEX_SEED = `# Knowledge

_空知识库。把页写进 \`wiki/\`,再跑 \`withy knowledge index\` 重算本目录。_
`;
...
// Scaffold an empty knowledge base under `<withyDir>/knowledge/`: sources/ + wiki/
...
function seedKnowledgeBase(withyDir: string, createdPaths: string[]): void {
  const base = resolve(withyDir, 'knowledge');
