...
## 10. 端到端流程串联(初始化 → 会话 → 任务 → 收尾)

> 本节把 §0–§9 的局部规则串成一条端到端链路(原 `harness-flow` 页并入);规则与 schema 仍以 [core.md](./core.md)、[cli.md](./cli.md)、[knowledge-base.md](./knowledge-base.md) 为准,本节只做跨域串联。最重要的不变量:**agent 自称完成 ≠ 节点完成;只有 `@withy/core` 的门禁通过后,workflow 才推进。**

分工:用户(说明目标/答疑/审批/授权跳过)· agent(执行当前 skill、产出产物、报告分支判断)· `withy` CLI/hook(统一事件入口,解析后调 core)· `@withy/core`(唯一事实与判定层:读写状态、算上下文、校验门禁、推进 workflow)· workflow(声明这一步要哪些产物与检查)· skill(这一步怎么做)· web(只读展示与编辑,不复制 core 判定)。
...
- [[core]] · [[knowledge-base]] · [[cli]] · [[decisions]]
