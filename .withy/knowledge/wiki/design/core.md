...
- **注全文 vs 注索引**:`context.json` 只说「注哪些知识 id、按哪步」,**注入形态(全文/索引)由各知识条目的 `inject: full | index` 字段决定**(knowledge-base.md §4/§7)。产物模板、必读短规范等注全文,规范库等长文档注索引(`title+summary+路径`),`resolvePlannedContext` 据此返回带形态的清单(harness §4/§6.4)。
- **会话须知(guide.md)不在此列**:它是工具文件 `.withy/guide.md`,session-start 直接读取注全文,不走 context.json/知识库(§2.2、harness §6.4)。
