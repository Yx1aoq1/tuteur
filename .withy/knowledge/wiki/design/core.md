...
- **定位 = 实施计划 + web 只读进度**,文件存在性可进 artifact 门禁,但勾选状态不参与节点放行(完成度的强约束在归档时内置校验,§9.2)、不做跨任务统计。
...
- **守红线**:gate 只把 `implement.md` 当普通 artifact 核「存在 + 非空」(L1);**勾没勾不参与节点放行**。勾选完成度的强约束放在**归档**——`withy task archive` 内置校验实施清单无未勾选项才放行(空清单视为通过,§9.2),而非在每个节点挂 checklist 门。
...
withy task archive <id> [--cancelled]  /  web 归档按钮
  → core.archiveTask(scope, id, { markCancelled? }):
      ...
      2. 终态校验(非 --cancelled 时):
         ...
         b. implement.md 的实施清单无未勾选项(否则报错,列出剩余条数)
         空清单(无复选框)视为通过,避免无清单任务被永久锁死
