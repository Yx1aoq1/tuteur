---
id: visual-design
title: 'Web 视觉设计规范(砚墨 Ink-stone)'
scope: project
kind: spec
tags: [withy, design, theme, tailwind, color-token, typography]
summary: 'web 视觉规范:砚墨双主题、配色 token 权威色值、字号阶梯、间距/圆角、组件样式契约、排版规则、globals.css 单一来源。'
inject: index
...
# Web 视觉设计规范(砚墨 Ink-stone)

本文定义 `packages/app`(`@withy/app`,Next.js 16 + React 19 + Tailwind 4)视觉层**长什么样**:视觉主题、配色 token、字号、间距、组件样式、排版规则。

分工:[web.md](./web.md) 管页面、数据流、API、交互;本文管观感。两者冲突时,功能行为以 web.md 为准,观感以本文为准。
...
- **③ 实施计划(implementation)**:从 `checklist.json` 渲染进度;每项 17px 复选框(`rounded-sm2`,`done` = `teal` 实心勾);已完成项文字 `line-through` + `text-ink-soft`;下方接进度条。
