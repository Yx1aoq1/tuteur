...
| `task status [task]`           | 看单任务的当前节点 / 阶段 / skill / 已完成节点 / 判定 / 实施进度 / git(task 缺省走当前任务)。**配了 `agent` 的节点额外带 `dispatch` 块 `{role, activeTask, curated, action}`**(2026-06-28,经 `relayNext` 现读 dispatch.json 算 `curated`;无 `--role`,core §interpret)                                                                                                                                       | `--task <id>`                                                                                                                                          | `{ ok, task, title, status, node, phase, skill, agent?, dispatch?, completed[], decisions, artifacts, git, nextAction }`(`dispatch` 仅配 agent 节点带;`nextAction` 软引导)            |
...
> **子 agent 角色定义投递(2026-06-28 派遣轮,已落地)**:canonical 角色定义 **`.agents/agents/<role>.md`**(单 md,与 skill 同 `.agents/` 提交家)由 **core `deployAgents`**(非 cli configurator——web 也要投递,故下沉 core)按目标工具格式投递:Claude `.claude/agents/<role>.md`(文件级软链)、Codex `.codex/agents/<role>.toml`(md→toml 生成);**格式驱动**,目标/格式由注册表每平台 `agentDef{target, format}` 派生,新工具加一条 `agentDef`(复用 handler)或一个 format handler。init 先 `installCanonicalWorkflowAgents` 铺 canonical(预置 implement/review/research,`templates/common/agents/<role>.md`),再在配置完各平台后调 `deployAgents(scope)` 投递;web agents 保存也调 `deployAgents`。子 agent **直接 `Read .withy/tasks/<id>/dispatch.json`**(扁平清单、无 `--role`,§3.1、core §4.5);`discoverAgents`/`resolveAgentRef`/`agentExists` 供 web agents 页与节点 `agent` 字段校验(harness §7.2)。**init 不再写 `context.json`**(§4.5 取消)。
...
| 写入者                                                | 文件                                                                                                                                                                                                      |
| ----------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `init`                                                | `config.yaml`、`guide.md`、`workflows/default.workflow.json`、`agents/<role>/`(canonical 角色定义,2026-06-28)、`template-hashes.json`、`.developer`、`workspace/<slug>/index.md`、`.gitignore`、`knowledge/{sources,wiki,index.md,log.md}`(**不再写 `context.json`**,core §4.5) |
| `task start` / `next` / `approve` / `rewind` / `note` / `checklist` / `hook` | `tasks/<id>/task.json`、`tasks/<id>/state.json`(含 `decisions`/`approvals`)、`tasks/<id>/events.jsonl`、`tasks/<id>/checklist.json`、`tasks/<id>/dispatch.json`(派遣必读种壳,2026-06-28)、`runtime/current-task.json`、`runtime/sessions/<sid>.json` |
| agent(文件工具直接写)                                 | `tasks/<id>/prd.md`、`tasks/<id>/design.md` 等节点产物;curate `tasks/<id>/dispatch.json` 真条目(实施计划改由 `withy checklist` 写 `checklist.json`,§3.3)                                                                                                          |
