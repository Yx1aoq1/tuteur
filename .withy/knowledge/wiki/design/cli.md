...
> 数据 schema 见 [core.md §4](./core.md);双层模型见 [core.md §2](./core.md);端到端串联见 [harness.md §10](./harness.md#10-端到端流程串联初始化--会话--任务--收尾)。
...
| C2  | `task start`(建或聚焦合一)/`list`/`status`/`archive`(归属/快速失败/YYYY-MM 分桶)               | ✅ 已实现(create/start 合一为 start;assign 已删,改派手编 task.json)       |
...
| C8  | `knowledge graph/index/lint/related/coverage`(分 scope;`covers` 文档↔代码 + `graph.json` 派生缓存)         | ✅ 已实现                                                                 |
| C9  | 全局 `--json` flag + 每命令人读渲染(缺省人读、`--json` 转结构化)                               | ❌ 待实现(当前 agent 命令恒 JSON)                                         |
| C10 | `checklist.json` 承载实施计划与进度,由 `withy checklist` 命令托管 + `readProgress` 唯一进度源(取代 implement.md,§3.3、[[node-gate-checkers]])           | ✅ 已实现                                                                 |
| C11 | `workflow validate` CLI 命令删除(`validateWorkflow` 仍作 core 函数,create/web 在用)            | ✅ 已实现(无 workflow CLI 命令;validateWorkflow 仍由 task start/web 调用) |
...
- [[core]] · [[harness]] · [[decisions]] · [[node-gate-checkers]] · [[task-event-timeline]]
