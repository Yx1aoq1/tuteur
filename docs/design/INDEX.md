...
| [cli.md](./cli.md)                   | `ttur` 命令(`init`/`task`/`next`/`approve`/`rewind`/`hook`/`knowledge`/`dashboard`/`update`/`uninstall`,缺省人读、`--json` 转结构化)、数据注册表+per-agent configurator+通用层(hook 走模板树)、模板更新 | 改命令、加 agent 平台、做 hook 入口、skill 发现时                  |
...
| workflow 校验 `validateWorkflow`(无环/唯一 default/边存在/阶段单调/skill·template ref)+ `task start` 新建拦截 + web 保存校验 | 🟡 校验逻辑 + create 拦截已实现;独立 `ttur workflow validate` 命令待删(校验留作 core 函数)                                               | harness §3、H10                  |
...
P0(主闭环,先地基后闭环)
  ...
  5. ttur task start(不存在则新建)/list/status/archive(归档分桶)     cli C2 / core §9 / harness H7
