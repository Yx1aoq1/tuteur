...
### 3.3 workflow 画布编辑器(已实现)

画布 = **三个固定阶段容器**(规划 / 执行 / 收尾,不可增删)+ 容器内的两类节点(skill / switch,core §4.3)。技术选型 **React Flow(`@xyflow/react`)的 Sub Flow**(`parentId` + `extent:'parent'`):三个容器是预置父节点(不可删/改名),拖入即归属该阶段;连线由 `next`/`branches` 推导,**无自由连线引擎**。本轮仅编辑项目的 `default` workflow。

```text
            ┌─ 规划 ───────┐   ┌─ 执行 ──────┐   ┌─ 收尾 ─┐
            │ triage(分支) │   │ dev → check │   │ wrapup │
            │ → brainstorm │──→│             │──→│        │
            │ → grill-me   │   │             │   │        │
            └──────────────┘   └─────────────┘   └────────┘
```

布局:三容器左→右并排,容器内节点按出现序自上而下堆叠(确定性自动布局,不存坐标)。**无独立前置泳道** —— 入口节点也住在某个容器里,阶段缺省/未声明的节点归入第一个容器显示。节点不可拖动改位(布局由结构推导),但**出口可连线**。

- **节点来源(右栏列表,可拖入)**:特殊「分支(switch)」节点 + `discoverSkills` 发现的 skill(按逻辑名去重、带 `source` tag,core §5.1)。拖一项到某阶段容器即在该阶段新建节点。
...
- **出口连线设定走向**:skill 一个出口、switch 每条分支各一个出口;从出口拖到目标节点入口即写回 `next` / `branch.next`,改连 / 删连同步更新,**不再用下拉选 next**。同阶段走纵向、跨阶段走横向(正交 smoothstep);入口连接点隐形以减噪。
- **右侧面板(docked,宽度对齐看板详情 336px)**:未选中=可拖入的节点列表;选中=该节点配置表单,顶部「返回」回列表(非悬浮)。
- 保存 `PUT /api/workflows/:id`(URL 的 id 为写文件名的权威来源),经 core 校验(zod schema → 连通 / 无环 / 阶段单调 / switch default / skill 可解析);结构 error 拒绝落盘(422 带 issues),skill / 模板悬空为 warning(允许保存、随响应回传提示)。core 侧新增 `writeWorkflow`(schema 校验后落盘,store.ts)。
...
| W10 | workflow 画布编辑(三固定容器 + skill/switch 节点,React Flow Sub Flow)+ `GET\|PUT /api/workflows` ✅已实现 | P1     | core §4.3、§3.3             |
