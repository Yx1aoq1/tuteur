...
packages/cli/src/
  ...
  configurators/
    registry.ts           # [已实现] AGENT_PLATFORMS 数据 + PLATFORM_CONFIGURATORS 行为表 + configureAgentPlatform 派发
    shared.ts             # [已实现] copyAgentTemplates/resolveWorkflowSkills/writeSkills/linkSkills/copyCanonicalSkills/占位符
    codex.ts claude.ts gemini.ts  # [已实现] 每平台 configurator(拷模板树 + skill;hook 脚本正文待填)
    index.ts              # [已实现] 聚合 re-export
  installation/
...
退出码约定:`0` 成功 / `1` 通用错误 / `2` **门禁失败** / `3` 用户取消。所有查询与门禁命令支持 `--json`(成功与失败都输出结构化结果,退出码语义不变;skill 模板指示 agent 优先用 `--json`)。

### 5.1 `ttur task ...`

```text
ttur task create "<title>" [--workflow <id>] [--assignee <slug>]
ttur task list [--mine|--all] [--status ...] [--json]
ttur task status [<task>] [--json]
...
`create`:`core.resolveProjectScope()` → 校验 workflow → `taskId=<MM-DD>-<slug>` → 写归属(`creator = 当前 .developer.slug`,`assignee = --assignee ?? 当前 .developer.slug`;**既无 `.developer` 又无 `--assignee` 则快速失败**,提示先 `ttur init -u`,对齐 Trellis,core.md §3.1)→ `core.writeTask/writeState`(state 初始化到 `entry` 节点);同名检测只查活跃任务目录(归档区按 `<YYYY-MM>` 分桶,跨年同名可共存,core.md §9)。`start`:写当前任务指针,完成/归档自动清除(harness §7.1)。`assign`:只改 `assignee`(校验 `workspace/<slug>/` 在册,core.md §3.1)。`list --mine` 用 `core.isOwnedBy`(按 assignee)+ `core.shouldFilterByUser`(全局根不过滤,core.md §3),默认不含归档。`archive`:`core.archiveTask` —— 默认不改 status(未完成任务询问,或 `--cancelled` 标记取消)+ 写 archivedAt + 移动目录到 `tasks/archive/<YYYY-MM>/<id>/`,**不绑任何产物、不执行任何 git 操作**(core.md §9)。

### 5.2 `ttur complete <node>` —— 核心门禁

```text
ttur complete <node> [--task <task>] [--json] [--skip --reason "..."]
  # node 必须是 skill 节点(decision 由 harness 自动推进);--task 缺省走 resolveCurrentTask(harness §7.1)
  → core.completeNode(scope, taskId, node)
  → result.ok  → exit 0 + stdout 下一节点接力块(harness §2.3:下一节点/产物/上下文/下一条命令;decision 路由原因)
  → !result.ok → exit 2 + 缺失项(--json 时结构化)
...
ttur hook inject-subagent-context  # 派生子 agent:输出/改写该节点精选上下文(P1+;pull 平台不发)
  ...
  公共参数/行为(harness §6.2):
    ...
    --json            结构化输出(信封形态见 harness §6.2 矩阵)
...
已实现并由 `init` 写出的:`config.json`、`context.json`、`workflows/default.workflow.json`、`template-hashes.json`、`.developer`、`workspace/<slug>/index.md`。
待实现:`tasks/<id>/{task,state}.json`、`events.jsonl`、`approvals.json`、`runtime/current-task.json`。
...
workflow 编排 skill,需要列出本地都有哪些 skill。**这是 core 的读能力,不是一条 `ttur` 命令**——消费方是 web 画布:定义 workflow 时,skill 节点的 `skillRef` 下拉从这里取候选、按 `agent`/`source` tag 分组(web §3.3、core §5.1)。CLI 不为此单开命令(对齐 Trellis:其 CLI 也只有 init/uninstall/update)。

发现目录由注册表 `skillDirs` 的静态两组 `{project, global}` 声明:project 组相对项目根、global 组相对各 agent 的 **home 目录**(全局 skill 不在 `~/.tuteur/`,core §2.3 安全边界)。扫描逻辑落在 core(`store.discoverSkills`,core §5.1),内部遍历 `AGENT_PLATFORMS` 的 `skillDirs` + canonical `.agent/skill`,解析 frontmatter 后去重并带 `agent`/`source` tag:

```ts
// core/store.ts(节选,完整签名见 core.md §5/§5.1)
export function discoverSkills(scope: Scope): DiscoveredSkill[] {   // 项目 scope
  const out: DiscoveredSkill[] = [];
  for (const tool of Object.values(AGENT_PLATFORMS)) {
    for (const dir of tool.skillDirs.project)            // 相对项目根(.claude/skills 等)
      out.push(...scanSkillDir(scope.root, dir).map(s => ({ ...s, agent: tool.id, source: 'project' as const })));
    for (const dir of tool.skillDirs.global)             // 相对用户 home(~/.claude/skills 等;无则空数组)
      out.push(...scanSkillDir(os.homedir(), dir).map(s => ({ ...s, agent: tool.id, source: 'global' as const })));
  }
  out.push(...scanCanonical(scope));                     // .agent/skill canonical(项目)
  return dedupe(out);
}
```

web 经 `GET /api/skills?project`(web §3.3/§9 W11)拿到发现结果;`resolveSkillRef` 用同一数据做校验期检查。注册表只提供 `skillDirs` 这类**静态目录**,扫描与解析是 core 的事。
...
| C3 | `complete <node>`(退出码 0/2、成功接力输出、`--json`、`--skip` 留痕) | P0 | core K4、harness §2.3-§2.4 |
...
| C11 | `workflow validate`(节点连通/无环/skillRef/decision default) | P2 | core resolveSkillRef |
