...
当前缺口:workflow 类 5 个 SKILL.md(brainstorm/grill-me/dev/check/finish)+ 维护类 `withy-knowledge`(知识库维护,knowledge.md §5)全是 `TODO:` 占位。门禁可先于 skill 内容落地,但端到端跑通前必须填实(§9 H6)。`withy-knowledge` 不是 workflow 节点,而是按需调用的维护 skill,不进默认 workflow。(注:默认 workflow 的分诊 `triage` 是 **switch 节点、不引用 skill**,故无需 classify skill。)
...
- 旧门禁路径(`completeNode`:skill gate / switch `--branch`)、`advanceWorkflow`、`rewindTo`、`approveNode`、`resolveCurrentTask`、`withy hook session-start` 已落地并跑通闭环;**剩余缺口是迁移为 `withy next`/`nextNode`、删除 `complete`、5 个 SKILL.md 正文仍为 TODO**(端到端体验差最后一环),以及 per-turn / 子 agent hook、平台信封适配未做。
