...
- **NO ACTIVE TASK** → 引导描述需求让 **agent 建任务**(`ttur task start "<title>"`,不存在则新建,core §3.1)
...
export function runSessionStartHook(): number {
  ...
  if (taskId) {
    ...
    out.push('- NO ACTIVE TASK · Next-Action: 描述需求让我 ttur task start "<title>"\n');
  }
...
- 内容:`当前任务 · 当前节点 · 主体阶段 · Next-Action: ttur next`;无活跃任务时给「描述需求后 `ttur task start`」的提示。若当前节点是 switch,Next-Action 带 `--branch <label> --reason "..."`。
